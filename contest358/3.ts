...
class BinaryTreeNode<T = any> {
  ...
  previousLowerNode(value: T): null | BinaryTreeNode<T> {
    ...
    if (this.nodeValueComparator.lessThan(this.value, value))
      return this.right
        ? this.right.previousLowerNode(value) ?? this
        : this
    return null
  }
  nextBiggerNode(value: T): null | BinaryTreeNode<T> {
    ...
    if (this.nodeValueComparator.greaterThan(this.value, value))
      return this.left
        ? this.left.nextBiggerNode(value) ?? this
        : this
...
function minAbsoluteDifference(nums: number[], x: number): number {
  if (x <= 0 || nums.length <= 1) return 0
  let res = Number.MAX_SAFE_INTEGER

  // doesn't have sorted array, use method to simulate it.
  const sortedArr = []
  ...
  for (let i = 0; i < nums.length; i++) {
    if (i >= x) {
      ...
      // O(n * log(n))
      avlTree.insert(nums[i - x])
      if (avlTree.contains(nums[i])) return 0
      res = Math.min(res,
        nums[i] - (avlTree.root.previousLowerNode(nums[i])?.value ?? (nums[i] - res)),
        (avlTree.root.nextBiggerNode(nums[i])?.value ?? (res + nums[i])) - nums[i]
        )

      // const index = lowerBound(sortedArr, nums[i - x])
      // O(n * n)
      // sortedArr.splice(index, 0, nums[i - x])
      //
      // const lowerBoundIndex = lowerBound(sortedArr, nums[i])
      // // 两边找最小 diff
      // // lowerBoundIndex: >= nums[i]
      // // lowerBoundIndex - 1: < nums[i]
      // if (lowerBoundIndex < sortedArr.length) res = Math.min(res, sortedArr[lowerBoundIndex] - nums[i])
      // if (lowerBoundIndex > 0) res = Math.min(res, nums[i] - sortedArr[lowerBoundIndex - 1])
      //
...
function lowerBound(numbers: number[], target: number, left: number = 0, right: number = numbers.length - 1): number {
  while (left <= right) {
