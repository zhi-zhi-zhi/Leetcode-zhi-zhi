...
function kthPalindrome(queries: number[], intLength: number): number[] {
  const upper = 9 * (10 ** ((intLength - 1) >> 1))
  const n = queries.length
  const res = Array(n).fill(-1)
  const basic = 10 ** ((intLength - 1) >> 1)


  queries.forEach((query, index) => {
    if (query <= upper) {
      let left = basic + query - 1
      let right = left
      // 奇数时，删掉一个
      if (intLength % 2 === 1) {
      ...
      // 翻转right，合并成结果
      // const result = parseInt(`${left}${Array.from(right + '').reverse().join('')}`)
      // res[index] = result

      for (let i = (intLength - 1) >> 1; i >= 0; i--) {
