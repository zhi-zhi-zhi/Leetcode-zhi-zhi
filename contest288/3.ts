 ...
 * @param k
 */
function maximumProduct(nums: number[], k: number) {
  const modulo = 1e9 + 7
  let queue = new PriorityQueue<number>((a, b) => a - b)
  nums.forEach(num => queue.add(num))

  if (queue.size() <= 0)
    return 0
  while (k > 0){
    const num = queue.deleteTop()!
    queue.add(num+ 1)
    k--
  }

  return queue.getOriginArr().reduce((a, num) => a * BigInt(num), BigInt(1)) % BigInt(modulo)
}