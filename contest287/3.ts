...
function maximumCandies(candies: number[], k: number): number {
  ...
  const allCandies = candies.reduce((sum, candy) => sum += candy, 0)

  if (allCandies < k)
    return 0

  let low = 1, high = Math.trunc(allCandies / k)
  ...
  function canAllocate(n: number): boolean {
    return candies.reduce((all, candy) => all += Math.floor(candy / n), 0)
      >= k
  }
}