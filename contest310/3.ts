...
function minGroups(intervals: number[][]): number {
  ...
  for (let i = 0; i < intervals.length; i++) {
    ...
    if (priorityQueue.size() > 0 && left > (priorityQueue.top()!)[1]) {
      priorityQueue.replaceTopAndAdjustment([priorityQueue.top()![0], right])
    } else if (priorityQueue.size() > 0 && left <= (priorityQueue.top()!)[1]) {
