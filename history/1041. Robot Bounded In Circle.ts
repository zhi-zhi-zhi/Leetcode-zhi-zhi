/**
 ...
 North
 West                East    , So do a counter clockwise assumption. If we are at a directions North, then its counter clockwis, yes West update direction by west, Same way if our directions is
 South                West, them its counter clockwise south, same way for direction south, update
 direction by east. So just rememebr if chaarcter is L, then counter clockwise.
...
function isRobotBounded(instructions: string): boolean {
  ...
  Array.from(instructions).forEach(instruction => {
    if (instruction === "G") {
      ...
      switch(directions) {
        case 'North': { directions = 'West'; break; }
        case 'West' : { directions = 'South'; break; }
        case 'South': { directions = 'East'; break; }
        case 'East' : { directions = 'North'; break; }
        default: // no nothing
      }
    } else if (instruction === 'R') {
      switch(directions) {
        case 'North': { directions = 'East'; break; }
        case 'West' : { directions = 'North'; break; }
        case 'South': { directions = 'West'; break; }
        case 'East' : { directions = 'South'; break; }
        default: // no nothing
      }
    }
  })

  return (x === 0 && y === 0) || directions !== 'North'
}